...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(interruptButton), handleButtonInterrupt, RISING);
}

void ICACHE_RAM_ATTR handleButtonInterrupt() {
  caraMode = !caraMode;
  Serial.print("current mode is caraMode? ");
  Serial.println(caraMode);
}
...
void loop()
{
  ...
  Serial.println(n);
  CO2bucket += CO2;
  double ff = ((CO2bucket / RMCO2divider - 400.0) / 1000000.0) / 0.038;
  Serial.println(ff, 4);
  ...
  PRMMaskOffD = (1 - exp(-1.0 * ff * infectors * qMaskOffD * t * 9.0 / n)) * 100.0;
...
void sendMQTT() {
  ...
  client.publish("[mqtt topic]", msg);
  snprintf (msg, 50, "%.4f", probValMaskOnD);
  client.publish("[mqtt topic]", msg);
  snprintf (msg, 50, "%.4f", probValMaskOffD);
  client.publish("[mqtt topic]", msg);
  snprintf (msg, 50, "%.2f", PRMMaskOnH);
  client.publish("[mqtt topic]", msg);
  snprintf (msg, 50, "%.2f", PRMMaskOffH);
  client.publish("[mqtt topic]", msg);
  ...
  doc["M2probMaskOnH"] = probValMaskOnH;
  doc["M2erMaskOnH"] = erValMaskOnH;
  doc["M2probMaskOffH"] = probValMaskOffH;
  doc["M2erMaskOffH"] = erValMaskOffH;
  doc["M2probMaskOnD"] = probValMaskOnD;
  doc["M2erMaskOnD"] = erValMaskOnD;
  doc["M2probMaskOffD"] = probValMaskOffD;
  doc["M2erMaskOffD"] = erValMaskOffD;
...
void showData(const char name[])
{
  ...
  if (PRMMaskOffD == 0) {
    ...
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.1);
    display.setFont(&FreeMonoBold9pt7b);
    display.print("Wing Hong OR");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.2);
    display.setFont(&FreeMonoBold9pt7b);
    display.print("Extending CARA with sensors");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.35);
    display.setFont(&FreeMono9pt7b);
    display.print("This Project was built upon");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.4);
    display.println("CERNS's COVID Airborne Risk");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.45);
    display.println("Assessment (CARA) software");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.5);
    display.println("modelling tool.");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.65);
    display.println("Support from Henriques. A &");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.7);
    display.println("Devine. J is acknowledged.");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.8);
    display.setFont(&FreeMonoBold9pt7b);
    display.print("Supervisor: Martin de. Jode");
    display.setCursor(GxEPD_WIDTH * 0.15, GxEPD_HEIGHT * 0.86);
    display.setFont(&FreeMonoBold9pt7b);
    display.print("M.Sc. Connected Environments");
    display.setCursor(GxEPD_WIDTH, GxEPD_HEIGHT * 0.85);
    display.setFont(&FreeMonoBold9pt7b);
    display.print("Centre for Advanced Spatial Analysis");
    display.update();
    delay(3000);
  }
  else {
